...
Le schéma part de l'utilisateur final et de la donnée qu'il envoie à l'application, jusqu'à la donnée brute (catalogue, historique), et nomme pour chaque brique le **service Azure** retenu (partenaire cloud de Fashion-Insta). Il couvre l'ensemble des besoins métiers touchant à la donnée ou à l'IA — pas seulement la brique de recommandation garde-robe validée par le PoC :

- la recommandation à partir de la garde-robe photographiée, par **recherche vectorielle** — l'approche B (attributs classifiés) de [`probleme-ml.md`](../01-poc/probleme-ml.md) sert de point de départ PoC, la cible production vise un embedder appris et un index vectoriel Azure AI Search (voir ci-dessous) ;
...
- la gestion des données personnelles associées à ces flux (détaillée dans [`Travail/03-donnees-personnelles/`](../03-donnees-personnelles/), représentée ici comme brique transverse car elle contraint le stockage et les pipelines qui consomment ces données).

Le PoC valide l'approche B (attributs classifiés → vecteur → recherche vectorielle). En production, l'encodeur évolue vers un **embedder appris directement sur l'image** (réseau de vision entraîné), capable de capturer des nuances stylistiques au-delà des attributs explicitement classifiés. Les vecteurs des articles du catalogue sont persistés et interrogés dans un **index vectoriel Azure AI Search** : il s'agit donc d'une vraie recherche de voisins les plus proches, et non d'un scan exhaustif du catalogue.
...
flowchart TB
    ...
    WARDROBE_RECO -->|article choisi| GEN["Azure ML / AI Foundry — GPU\n— virtual try-on génératif\n(Stable Diffusion)"]:::ml
```
...
| Entraînement et indexation | Azure Machine Learning pipelines | Entraîne et valide l'embedder ; calcule ensuite, en batch, les embeddings des articles avant chaque publication d'index. Point de départ PoC : classifieur d'attributs (catégorie, couleur, style, motif, coupe) formant ce vecteur (approche B). | Sépare le calcul coûteux et rejouable de l'inférence en ligne ; la version du modèle et celle de l'index restent traçables. |
...
| Virtual try-on génératif | Azure Service Bus + Azure ML / Azure AI Foundry (GPU) | Une demande est placée dans une file ; un worker GPU produit le rendu, l'enregistre dans Blob et met à jour son statut dans SQL. | Le traitement asynchrone lisse les pics, évite de bloquer l'API et permet de dimensionner les GPU indépendamment. Le cache évite de régénérer un essai identique. |
...
- Le virtual try-on génératif est le plus coûteux en calcul par requête : Azure Service Bus le traite de façon **asynchrone** ; l'application consulte le statut puis récupère le rendu, ce qui lisse la charge GPU plutôt que de dimensionner pour un pic temps réel.
